...
LC3::LC3(sc_module_name nm):sc_module(nm),m_N(0),m_Z(0),m_P(0),m_PC(0x3000),m_R(LC3_NUM_REGS)
{
...
void LC3::dec_and_exec(uint16 instruction)
{
	...
	case  OP_ADD: // DR = SR1 + SR2 or DR = SR1 + imm5
		{
			if(instruction & 0x20){
				...
				std::cout<<"Using IMM5 value because:"<<std::hex<<(instruction & 0x20)<<std::endl;}
			else
			{
	...
	case OP_LD:           // DR = mem[PCi + PCoffset9]
	{
		...
		dport->read(m_PC+SEXT(7,instruction & IMM9_MASK),data);
		std::cout<<"Loading data from M"<<std::hex<<m_PC+SEXT(7,instruction & IMM9_MASK)<<std::endl;
		m_R[(instruction&DR_MASK)>>9]=data;
		setcc(m_R[(instruction&DR_MASK)>>9]);
		std::cout<<"OP_LD result: "<<std::hex<<data<<" at R"<<((instruction&DR_MASK)>>9)<<std::endl;
		break;
	}
	...
	case OP_ST:           // mem[PCi + PCoffset9] = SR
	{
		...
		//trans=
				dport->write(m_PC+SEXT(7,instruction & IMM9_MASK),m_R[(instruction&DR_MASK)>>9] );
	...
	case OP_LDR:          // DR = mem[BaseR + offset6]
	{
		...
		//bus_if::status_t trans;
		//trans=
				dport->read(m_R[(instruction&BREG_MASK)>>6]+SEXT(10,instruction&IMM6_MASK), data);
		m_R[(instruction&DR_MASK)>>9]=data;
		setcc(data);
	...
	case OP_STR:          // mem[BaseR + offset6] = SR
	{
		//bus_if::status_t trans;
		//trans=
		dport->write(m_R[(instruction&BREG_MASK)>>6]+SEXT(10,instruction&IMM6_MASK),m_R[(instruction&DR_MASK)>>9]);
		break;
	}
	...
	case OP_LDI:          // DR = mem[mem[PCi + PCoffset9]]
	{
		...
		//bus_if::status_t trans;
		//trans=
		dport->read(m_R[m_PC+SEXT(7,instruction&IMM9_MASK)], data);
		m_R[(instruction&DR_MASK)>>9]=data;
		setcc(data);
		std::cout<<"OP_LDI result: "<<std::hex<<data<<" at R"<<((instruction&DR_MASK)>>9)<<std::endl;
		break;
	}
	...
	case OP_STI:          // mem[mem[PCi + offset9]] = SR
	{
		...
		//bus_if::status_t trans;
		//trans=
				dport->read(m_PC+SEXT(7,instruction&IMM9_MASK),tempaddr);

		//trans=
				dport->write(tempaddr,m_R[(instruction&DR_MASK)>>9]);
				std::cout<<"OP_LDI result: "<<std::hex<<m_R[(instruction&DR_MASK)>>9]<<" at M"<<std::hex<<tempaddr<<std::endl;
		break;
	}
...
void LC3::step()
{
	...
	while(1)
	{
		...
		std::cout<<"PC : "<<std::hex<<m_PC<<std::endl;
		m_PC+=1;
		dec_and_exec(instruction);
		wait(5,SC_NS);
		std::cout<<"OPCODE: "<< opcode_t(get_opcode(instruction))<<std::endl;
	}
}
